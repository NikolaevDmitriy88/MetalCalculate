...
void CalculateResult(HWND msgResult)
{
	...
	if (isTabularInfo)
	{
		if (!msgLengthOrWeight)
			result = std::stof(resultMsg) * kgInMeter / 100;	// Ввод сантиметры вывод кг, поэтому kgInMeter / 100 
		else
			result = std::stof(resultMsg) / kgInMeter;			// Ввод в кг вывод метры, если надо сантиметры умножай на 100
	}
	else
	{
		hight = inputHight;
